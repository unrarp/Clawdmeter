...
def main():
    ...
    ap.add_argument(
        "--scale", type=int, default=3, help="integer upscale factor (nearest, default 3)"
    )
    ap.add_argument(
        "--pad-to",
        metavar="WxH",
    ...
    print(f"wrote {out}  ({anim['name']}, {len(frames)} frames, {px}, {sum(durations)} ms loop)")
