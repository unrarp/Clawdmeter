...
- **The daemon-health verdict is event-driven, not ticked.** `net_daemon_health()` returns one **aggregate over both providers**, in priority order: `DAEMON_OFFLINE` (WiFi down) → `DAEMON_NEEDS_ACTION` (a provider's broker token needs user re-auth) → `DAEMON_BROKER_DOWN` (a token is needed but the broker didn't reply) → `DAEMON_NO_TOKEN` (online, fetching tokens) → then the data-freshness states `DAEMON_NO_DATA`/`DAEMON_CONNECTED`/`DAEMON_STALE` (off elapsed time since the last good provider fetch; `DAEMON_STALE_MS` ~2.75× `FETCH_INTERVAL_MS`). Auth/token states outrank freshness because they're actionable and explain missing data; the per-provider `ok`/`cok` wire flags still carry each panel's own state so one provider's problem never blanks the other. Because several verdicts are elapsed-time transitions no fetch announces, `loop()` polls it every iteration but repaints the WiFi page **only on a change** — the `last_daemon_health` latch in `main.cpp`. Don't reintroduce a fixed-interval WiFi refresh.
