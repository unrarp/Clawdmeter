...
## Decision / Solution
Keep pre-baked, per-size LVGL bitmap fonts (`lv_font_conv --format lvgl --bpp 4 --no-compress`, one `.c` per typeface+size; declared via `LV_FONT_DECLARE` in `ui.cpp`). Do not enable a runtime rasterizer.
